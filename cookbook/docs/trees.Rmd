...
One of the main features about XGBoost is that it has been able to build tree components in parallel. Like naive GBDT, It generates trees sequentially. However within a single tree, the child nodes are mutually independent. Thus each of these child nodes can be allocated to one core (or thread), to compute further split from the child node.
...
* GBDT only uses the first order derivative of the loss function, while XGBoost uses both the first and second order derivatives (by Taylor series). Second order derivative allows gradient to descend faster in the right direction.

* GBDT uses tree base learner, while XGBoost also supports other types such as linear learner. Though in most cases, we may not use linear learner as a base learner, because adding multiple linear models together still yields a linear model.

* Naive GBDT uses all data points in every iteration. XGBoost randomly samples the data to prevent over-fitting.
